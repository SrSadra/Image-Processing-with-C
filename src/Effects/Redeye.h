...
int removeRedEye(unsigned char* image, int width, int height , int channels ) {
    for (int i = 0; i < width * height; i++) {
        ...
        if (r > 200 && g < 150 && b < 150) {
            // Adjust color to reduce red-eye effect
            float ratio = (float)g / (float)r;
            image[i * channels] = (unsigned char)fmin(255.0f, r * ratio);
            image[i * channels + 1] = (unsigned char)fmin(255.0f, g * ratio);
            image[i * channels + 2] = (unsigned char)fmin(255.0f, b * ratio);
        }
    }
    return 0;
}
